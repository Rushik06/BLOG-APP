...
export default async function PricingPage() {
  ...
  const ctaTitle =
    proPlan?.ctaTitle || PRICING_DEFAULTS.ctaTitle;

  const ctaSubtitle =
    proPlan?.ctaSubtitle || PRICING_DEFAULTS.ctaSubtitle;

  const contactNumber =
    proPlan?.contactNumber || PRICING_DEFAULTS.contactNumber;

  const highlightTitle =
    proPlan?.highlightTitle || PRICING_DEFAULTS.highlightTitle;

  const highlightPoints =
    proPlan?.highlightPoints || PRICING_DEFAULTS.highlightPoints;
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-12">
        ...
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {plans.map((plan) => {
            ...
            return (
              <div
                key={plan.id}
                className="relative transition-all duration-300 hover:-translate-y-2 hover:shadow-xl rounded-2xl"
              >
                {isPopular && (
      ...
      {/* HIGHLIGHT  */}
      <div className="mt-20">
        <Card className="border-blue-100 rounded-2xl hover:shadow-lg transition-all">
      ...
      <div className="mt-16 text-center">
        ...
        <div className="mt-6 flex justify-center">
          <a
            ...
            className="flex items-center gap-2 rounded-full bg-blue-600 px-6 py-3 text-white shadow-md transition hover:bg-blue-700 hover:scale-105 active:scale-95"
          >
            <Phone size={16} />
            {contactNumber}
          </a>
